...
clip_username() {
  echo "$dbpass" | keepassxc_cli show "$db" "$entry" | grep 'UserName:' | \
    cut -b 11- | xclip -sel clip
}
clip_password() { echo "$dbpass" | keepassxc_cli clip "$db" "$entry" "$timeout" ;}
...
edit_element() {
  ...
  then
    ...
    echo "$dbpass" | keepassxc_cli edit -t "$new_title" "$db" "$entry"
  elif echo "$element" | grep 'UserName: '
  then
    new_username=$(m -p "Enter new entry username" -l 0 -width 400)
    echo "$dbpass" | keepassxc_cli edit -u "$new_username" "$db" "$entry"
  ...
  then
    ...
    case "$action" in
      "Enter password")
        ...
        printf "%s" "$dbpass\n$new_password" | keepassxc_cli edit -p "$db" "$entry"
        ;;
      "Generate password")
        generate_password
        echo "$dbpass" | keepassxc_cli edit -g -L "$(echo "$char_num $numbers$special$ext_ascii")" \
  ...
  then
    ...
    echo "$dbpass" | keepassxc_cli edit --url "$NEW_URL" "$db" "$entry"
...
show_entry_info() {
  element=$(echo "$dbpass" | keepassxc_cli show "$db" "$entry" | grep -Ev 'Enter|?*/' | m -p "Info" -l 7 -width 350)
...
delete_entry() {
  echo "$dbpass" | keepassxc_cli rm "$db" "$entry"
...
add_entry() {
  ...
  case "$action" in
    "Enter password")
      ...
      printf "%s" "$dbpass\n$password" | keepassxc_cli add "$db" "$entry" -u "$username" -p
      ;;
    "Generate password")
      generate_password
      echo "$dbpass" | keepassxc_cli add "$db" "$entry" -u "$username" -g -L \
...
check_pass=$(echo "$dbpass" | keepassxc_cli open "$db" >"$CACHE/tmp" 2>&1 && grep -oh "$error_pass" "$CACHE/tmp")
error_db='Failed to open database file'
check_db=$(echo "$dbpass" | keepassxc_cli open "$db" >"$CACHE/tmp" 2>&1 && grep -oh "$error_db" "$CACHE/tmp")
...
if [ "$check_pass" = "$error_pass" ]; then
  ...
  echo "$dbpass" | keepassxc_cli ls "$db" | grep -Ev 'Enter|?*/' | sort >"$CACHE/tmp"
